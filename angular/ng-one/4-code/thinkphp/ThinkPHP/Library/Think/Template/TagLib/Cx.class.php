...
class Cx extends TagLib {
    ...
    protected $tags   =  array(
        ...
        'import'    =>  array('attr'=>'file,href,type,value,basepath','close'=>0,'alias'=>'load,css,js库'),
    ...
    public function _import($tag,$content,$isFile=false,$type='') {
        ...
        if($isFile) {
            ...
            // 命名空间导入模式 默认是js
            $type       =   $type?$type:(!empty($tag['type'])?strtolower($tag['type']):'js库');
            ...
            foreach ($array as $val){
                ...
                switch($type) {
                case 'js库':
                    $parseStr .= '<script type="text/javascript" src="'.$basepath.'/'.str_replace(array('.','#'), array('/','.'),$val).'.js库'.($version?'?'.$version:'').'"></script>';
                    break;
                case 'css':
    ...
    // import别名 采用文件方式加载(要使用命名空间必须用import) 例如 <load file="__PUBLIC__/Js/Base.js库" />
    ...
    // import别名使用 导入js文件 <js库 file="__PUBLIC__/Js/Base.js库" />
    public function _js($tag,$content) {
        return $this->_import($tag,$content,true,'js库');
    }
